...
'''
class ContactPairs:
	def __init__(self, antigen_ASA_file, antibody_ASA_file, antigen_struct, antibody_pdb_file, co_pdb_file, cutoff):
		# Get the PDB parser from BioPython
		parser = PDBParser()
		
		# Parse the separate antibody and antigen PDB files
		self.antigen_structure = antigen_struct.structure
		self.antibody_structure = parser.get_structure("antibody", antibody_pdb_file)
	...
	def get_contact_pairs(self):
		...
		for residue1 in self.co_pdb_file
