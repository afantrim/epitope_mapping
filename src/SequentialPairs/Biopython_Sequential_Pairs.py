...
class SequentialPairs:
	def __init__(self, peptide_file, asa_file, sequence_file, reduced):
		# Make the inputs instance variablles
		self.sequence_file = sequence_file
		self.asa_file = asa_file
		self.peptide_file = peptide_file
		
		# Read in the peptide file
		with open(peptide_file, "r") as file:
			self.peptides = file.read().split('\n')
			
		# Read in the ASA file
		self.surface_residues = ASAParse(self.asa_file).SA_dict
		
		# To be filled in in make_pairs()
		self.valued_spheres = defaultdict(int)
		
		# Optional section to translate the peptides to the reduced alphabet if the user so chooses
		if reduced:
		...
		# Call the make_pairs function to create the pair_dict, which breaks down
		# the set of peptides into sequential adjacent pairs
